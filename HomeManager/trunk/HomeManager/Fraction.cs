	...
	namespace Mehroz
	{
		...
		public struct Fraction : IComparable, IFormattable
		{
			...
			public static Fraction ToFraction(string inValue)
			{
				...
				{
                    string[] value = inValue.Split(' ');

                    int wholeNumber = 0;

                    if (value.Length == 2)
                    {
                        wholeNumber = int.Parse(value[0]);
                        inValue = value[1];
                    }
					...
					if (slashPos > -1)
					{
						...
						return wholeNumber + new Fraction(numerator, denominator);
					}
					else
					{
						...
						if (decimalPos > -1)
                            return wholeNumber + new Fraction(Convert.ToDouble(inValue));
						else
                            return wholeNumber + new Fraction(Convert.ToInt64(inValue));
					}
				}
			}
			#endregion
